...
The Flap and Pons Excel files have two visible sheets only: `控筹模型` and `Quote参数`. The control sheet groups editable scenario inputs first, budget results second, protocol/source information third, and detailed inner/outer/downside curves at the bottom. One Quote selector drives every result. A separate manual USDT price override defaults to blank; clearing it restores the dated pool-price lookup. Quote/USDT budgets exclude the separately displayed native Gas reserve. Financial formulas and existing defaults are preserved. Templates are authored and regression-checked locally with artifact-tool; cloud refresh continues to update price/provenance inputs only.

2026-09-18 layout refinement: control-sheet body rows are 20 pt; the reserve input is G12 under addresses/Gas. Protocol details (rows 30–46) are an initially collapsed native Excel outline with a visible section heading and expand control above. Results show the four Quote budget components, sunk-cost/inner-fill reference values, inner base fee, outer LP/Hook fee, tax-inclusive buy/sell rates, and their bilateral wash-budget sum. These rates retain the existing additive FDV budget assumption, not a new realized-trade-loss formula. Pons dynamic Snipe Tax remains excluded and its warning stays visible. Daily refresh preserves the outline and every non-price input/formula/style.
